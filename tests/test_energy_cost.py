...
class EnergyCostTests(unittest.TestCase):
    ...
    def test_gpu_to_wall_idle_and_load(self) -> None:
        ...
        self.assertAlmostEqual(
            energy_cost.gpu_to_wall(11, idle_wall_w=50, gpu_floor_w=11, slope=1.15), 50.0)
        # 20 W GPU → 50 + 1.15*(20-11) = 60.35
        self.assertAlmostEqual(
            energy_cost.gpu_to_wall(20, idle_wall_w=50, gpu_floor_w=11, slope=1.15), 60.35)
        self.assertIsNone(energy_cost.gpu_to_wall(None))
        # Never bill above the 240 W adapter
        self.assertEqual(energy_cost.gpu_to_wall(400, idle_wall_w=50, gpu_floor_w=11, slope=1.15),
                         energy_cost.WALL_CAP_W)
    ...
    def test_billable_integrate_higher_than_sensor(self) -> None:
        ...
        calib = {"idle_wall_w": 50, "gpu_floor_w": 11, "slope": 1.15, "pi_psu_eff": 1.12}
        ...
        # wall ≈ 50 + 1.15*(18-11) = 58.05 W → 0.05805 kWh vs GPU 0.018
        self.assertGreater(bill["kwh"]["node1"], sens["kwh"]["node1"] * 2)
        # Pi wall = PMIC × 1.12
        self.assertAlmostEqual(bill["kwh"]["pi"], sens["kwh"]["pi"] * 1.12, places=5)
    ...
    def test_integrate_window(self) -> None:
        ...
        # node2 missing after hour 1 — do NOT invent a second hour of 50 W
        self.assertAlmostEqual(out["kwh"]["node2"], 0.05, places=5)

    def test_onesided_short_hold(self) -> None:
        samples = [
            {"t": 0, "w": {"node1": 100}},
            {"t": 30, "w": {}},
        ]
        out = energy_cost._integrate(samples, 0, 30, mode="sensor")
        # 100 W × 30 s = 0.000833 kWh
        self.assertAlmostEqual(out["kwh"]["node1"], 100 / 1000 * 30 / 3600, places=6)
    ...
    def test_pace_from_24h(self) -> None:
        w24 = {
            ...
            "nodes": {
                ...
                "node2": {"kwh": 1.0, "hours_covered": 24.0},
        ...
        # Incomplete sibling must not inflate via hours_sampled
        w_partial = {
            "nodes": {
                "node1": {"kwh": 1.0, "hours_covered": 24.0},
                "node2": {"kwh": 0.5, "hours_covered": 12.0},
            },
        }
        p2 = energy_cost._pace_from_24h(w_partial, days=30.0)
        self.assertAlmostEqual(p2["nodes"]["node2"], 15.0, places=4)
        self.assertAlmostEqual(p2["sparks_kwh"], 45.0, places=4)
