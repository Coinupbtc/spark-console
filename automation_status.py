...
def systemd_timers() -> list[dict]:
    ...
    try:
        ...
        rows = []
    units = [r.get("activates") for r in rows if r.get("activates")]
    results: dict[str, dict] = {}
    if units:
        show = _run(["systemctl", "--user", "show", *units,
                     "-p", "Id", "-p", "Result", "-p", "ActiveState", "-p", "ExecMainStatus"])
        block: dict[str, str] = {}
