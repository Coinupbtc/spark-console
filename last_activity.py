        ...
        for p in (
            ...
            if p.is_file():
                try:
                    ...
                    pass
        # Real traffic is Hermes deep clients (light + smeagle → :8100 / Puzzle).
        # Without this, a long-lived trial looks "13h unused · STOP?" while Kanban
        # and Telegram are actively hitting the lane through those profiles.
        for profile in ("light", "smeagle"):
            base = HOME / f".hermes/profiles/{profile}"
            for rel, label in (
                ("logs/agent.log", f"{profile}/agent.log"),
                ("state.db-wal", f"{profile}/state.db-wal"),
                ("state.db", f"{profile}/state.db"),
                ("auth.json", f"{profile}/auth.json"),
            ):
                p = base / rel
                if p.is_file():
                    try:
            ...
            if ping.is_file():
                try:
                    ...
                    if best_ping:
                        sources.append((float(best_ping), f"{profile}/kanban ping"))
                except Exception:
                    pass
